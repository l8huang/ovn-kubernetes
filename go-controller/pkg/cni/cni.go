...
func (pr *PodRequest) ensureNetworkEncapIP(clientset *ClientSet, pod *corev1.Pod, podNADAnnotation *util.PodAnnotation) error {
	...
	if len(encapIP) > 0 {
		klog.Infof("%s use encap IP %s", pr, encapIP)
...
func (pr *PodRequest) cmdAddWithGetCNIResultFunc(
	...
	if primaryUDNPodRequest != nil {
		...
		klog.Infof("XXXX cmdAdd: primry network %s, podNADAnnotation.EncapIP %s, podInterfaceInfo.EncapIP %s", primaryUDN.NADName(), primaryUDN.Annotation().EncapIP, primaryUDNPodInfo.EncapIP)
	...
	klog.Infof("XXXX cmdAdd: network %s, podNADAnnotation.EncapIP %s, podInterfaceInfo.EncapIP %s", pr.netName, podNADAnnotation.EncapIP, podInterfaceInfo.EncapIP)
