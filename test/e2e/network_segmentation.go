...
var _ = Describe("Network Segmentation", feature.NetworkSegmentation, func() {
	...
	Context("layer3 primary network with multi-subnets", func() {
		DescribeTableSubtree("created using",
			func(createNetworkFn func(netConfig *networkAttachmentConfigParams) error) {
				...
				DescribeTable(
					...
					func(netConfig *networkAttachmentConfigParams) {
						...
						GinkgoT().Logf("XXXX ipv4: %v, ipv6: %v", ipv4, ipv6)
						...
						GinkgoT().Logf("XXXX ipv4: %v, ipv6: %v", cidr2nodev4, cidr2nodev6)
						...
						By("ensure 2 scheduable Nodes on different CIDR")
						nodeList, err := e2enode.GetReadySchedulableNodes(context.TODO(), cs)
						framework.ExpectNoError(err)
						...
						GinkgoT().Logf("XXXX networkName: %s", networkName)

						node2cidrv4 := map[string]string{}
						node2cidrv6 := map[string]string{}
						nodes := []*v1.Node{}
						for _, node := range nodeList.Items {
							GinkgoT().Logf("XXXX node annotation: %s", node.Annotations["k8s.ovn.org/node-subnets"])
							ipnets, err := ovnkubeutil.ParseNodeHostSubnetAnnotation(&node, networkName)
							Expect(err).NotTo(HaveOccurred())
							GinkgoT().Logf("XXXX node %s ipnets: %v", node.Name, ipnets)
						...
						GinkgoT().Logf("XXXX node2cidrv4: %v, node2cidrv6: %v", node2cidrv4, node2cidrv6)
						...
						Expect(clientNodeName != "" && serverNodeName != "").To(BeTrue(), "no pair of nodes with different IPv4 and IPv6 CIDRs found")
						...
						Expect(err).NotTo(HaveOccurred())
						GinkgoT().Logf("XXXX clientNodeName: %s, clientIPs: %s ", clientNodeName, clientIPs)
						...
						Expect(err).NotTo(HaveOccurred())
						GinkgoT().Logf("XXXX serverNodeName: %s, serverIPs: %s ", serverNodeName, serverIPs)
			...
			Entry("UserDefinedNetwork", func(c *networkAttachmentConfigParams) error {
				GinkgoT().Logf("XXXX createNetworkFn: %v", c)
				udnManifest := generateUserDefinedNetworkManifest(c, f.ClientSet)
				GinkgoT().Logf("XXXX udnManifest: %s", udnManifest)
				cleanup, err := createManifest(c.namespace, udnManifest)
				GinkgoT().Logf("XXXX createManifest DONE")
				DeferCleanup(cleanup)
				// sleep 500 seconds

				Eventually(userDefinedNetworkReadyFunc(f.DynamicClient, c.namespace, c.name), 500*time.Second, time.Second).Should(Succeed())
				return err
			}),
			Entry("ClusterUserDefinedNetwork", func(c *networkAttachmentConfigParams) error {
				cudnName := randomNetworkMetaName()
		...
		DescribeTableSubtree("created using",
			func(createNetworkFn func(netConfig *networkAttachmentConfigParams) error,
				...
				It("add subnet not affecting existing node subnet assignment", func() {
					...
					Expect(err).NotTo(HaveOccurred())
					Expect(len(oldv4) == 0 || len(oldv4) < len(nodeList.Items)).To(BeTrue(), "expect some nodes do not have subnet")
					Expect(len(oldv6) == 0 || len(oldv6) < len(nodeList.Items)).To(BeTrue(), "expect some nodes do not have subnet")
					GinkgoT().Logf("XXXX oldv4: %v", oldv4)
					GinkgoT().Logf("XXXX oldv6: %v", oldv6)
					...
					Eventually(func() error {
						...
						GinkgoT().Logf("XXXX newv4: %v", newv4)
						GinkgoT().Logf("XXXX newv6: %v", newv6)
						Expect(err).NotTo(HaveOccurred())
						if (len(newv4) == 0 || len(newv4) == len(nodeList.Items)) && len(newv6) == 0 || len(newv6) == len(nodeList.Items) {
							return nil
						}
				...
				It("add bad subnet should not cause NAD being updated", func() {
					By("creating the intial network with one CIDR")
					netConfig := &networkAttachmentConfigParams{
						name:      randomNetworkMetaName(),
						namespace: f.Namespace.Name,
					...
					Expect(err).NotTo(HaveOccurred())
					networkName := nad.Annotations["k8s.ovn.org/network-name"]
					GinkgoT().Logf("XXXX nad: %s", nad.Spec.Config)
					GinkgoT().Logf("XXXX networkName: %s", networkName)
			...
			Entry("UserDefinedNetwork",
				func(c *networkAttachmentConfigParams) error {
					udnManifest := generateUserDefinedNetworkManifest(c, f.ClientSet)
					GinkgoT().Logf("XXXX create udnManifest: %s", udnManifest)
					cleanup, err := createManifest(c.namespace, udnManifest)
					DeferCleanup(cleanup)
				...
				func(c *networkAttachmentConfigParams) error {
					udnManifest := generateUserDefinedNetworkManifest(c, f.ClientSet)
					GinkgoT().Logf("XXXX update udnManifest: %s", udnManifest)
			...
			Entry("ClusterUserDefinedNetwork",
				func(c *networkAttachmentConfigParams) error {
					cudnManifest := generateClusterUserDefinedNetworkManifest(c, f.ClientSet)
					GinkgoT().Logf("XXXX create cudnManifest: %s", cudnManifest)
					cleanup, err := createManifest("", cudnManifest)
					DeferCleanup(func() {
						cleanup()
						By(fmt.Sprintf("delete pods in %s namespace to unblock CUDN CR & associate NAD deletion", c.namespace))
				...
				func(c *networkAttachmentConfigParams) error {
					cudnManifest := generateClusterUserDefinedNetworkManifest(c, f.ClientSet)
					GinkgoT().Logf("XXXX update cudnManifest: %s", cudnManifest)
